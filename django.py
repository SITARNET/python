...
# названия полей можно посмотреть в админке -> код страницы

# добавление ссылки на форму регистрации и создание URL-схемы
# создание представления для регистрации
# создание шаблона register.html для отображения формы регистрации на странице
...
# djbook.ru/rel3.0/topics/cache.html
# Кэширование на файловую систему - самая популярная
# settings.py -> CACHES = {...}

# Кэширование на уровне представлений
# Кэш главной страницы -> urls.py -> path('', cache_page(60)(WomenHome.as_view()), name='home')
...
# Закэшируем в utils.py -> class DataMixin -> cats = Category.objects.annotate(Count('women'))
...
# coolsite -> templates -> admin -> base_site.html
# coolsite/settings.py -> TEMPLATES -> 'DIRS': [os.path.join(BASE_DIR, 'templates')]
# women/static/css/admin.css
# {% load static %}
# {% block extrastyle %}
